...
function getAverage(num1, num2) {
  const bNum1 = BigInt(num1);
  const bNum2 = BigInt(num2);
  const sum = BigInt(bNum1 + bNum2);
  const divider = BigInt(2);

  return Number(sum / divider);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const yDef = 0;
  const xDef = 0;
  const ang1 = Math.atan2(y1 - yDef, x1 - xDef);
  const ang2 = Math.atan2(y2 - yDef, x2 - xDef);
  return Math.abs(ang2 - ang1);
}
...
function isPowerOfTwo(num) {
  for (let i = 1; i <= Math.sqrt(Math.trunc(num)); i += 1) {
    if (2 ** i === num) {
      return true;
    }
  }
